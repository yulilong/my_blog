...
这个规则提示了IE11浏览器不支持`Headers`，这个可以点击`Quick Fix...`然后禁用这行的检查即可。
...
一个组件一个文件夹，子组件在自己的文件夹中，通用的组件放到公共文件夹
