...
### 1.2 查看开发者提交记录：git log --author=jack
...
3、显示每次提交修改的代码：`git log -p --author="jack"`
...
当commit只是在本地，没有推送到仓库中。发现commit功能有问题
...
#### 4.2.2 有提交信息的回退：git revert

由于代码已经在其他地方有了修改并提交了，或者一些环境不能破坏性的退回(git reset)。

此时可以使用`git revert`命令回退代码。revert命令回退指定的commit提交。revert命令会产生一个新的commit信息，就像做了一次修改一样。使得其他环境可以正常的拉取回退代码，而不会导致冲突。
...
## 10. git count-objects：已解压对象文件的数量及其占用的磁盘空间
