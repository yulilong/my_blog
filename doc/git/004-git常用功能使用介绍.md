...
## 14. 检查b分支是否已经合并了a分支的代码

1、 使用git branch --merged命令
...
# 如果 a 分支出现在列表中，说明已经合并
```

上面的验证后，还可以使用下面一些命令辅助验证更精确的验证。
...
# 查看在 a 分支上但不在 b 分支上的提交
git log b..a --oneline
# 如果没有输出，说明 a 的所有提交都已经在 b 中
# 如果有输出，说明这些提交还没有合并到 b

# 其他一些辅助验证
# 查看提交记录里面的提交信息是否有合并记录
git log --oneline --grep="Merge branch 'a'"
# 这个命令会输出在b分支中提交记录里面所有包含 Merge branch 'a'的信息，如果有合并，并且没有修改默认提交记录的话就可以看到合并记录
```



3、 merge-base --is-ancestor

```bash
# 切换到b分支
git checkout b
# 检查是否已经合并了a分支：输出 已合并 表示已经合并了
git merge-base --is-ancestor a b && echo "已合并" || echo "未合并"

# git merge-base --is-ancestor a b 命令的详细解释：
# 1、功能：检查分支 a 的提交是否是分支 b 的祖先提交（即 a 是否被 b 合并或包含）。
# 2、返回值：
#   状态码 0 ：表示 a 是 b 的祖先，b 包含了 a 的所有提交（已合并）。
#   非零状态码：表示 a 不是 b 的祖先，b 未包含 a 的提交（未合并）。
# 3、使用场景：
#   验证分支合并状态（如 a 是否已合并到 b）。
#   自动化脚本中判断分支继承关系。
# 注意：命令的输出结果通过‌退出状态码‌（exit code）传递，而非标准输出（stdout）
#      因此执行git merge-base --is-ancestor a b 后终端不会有任何输出
echo $? 
# 使用上面的命令可以查看输出的状态码
```

由于上面的只能验证 a分支是否是b分支的祖先提交。如果b分支的提交在a分支前面，那么就不能验证成功，需要使用下面命令再次验证。

4、使用 `git merge-base` 和 `git diff`

```bash
# 如果 a 分支的最新提交已经在 b 分支中
git merge-base a b
git rev-parse a

# 比较两个哈希值，如果相同说明已经合并
# 或者使用以下命令：
if [ "$(git merge-base a b)" = "$(git rev-parse a)" ]; then
    echo "a 分支的代码已经合并到 b 分支"
else
    echo "a 分支的代码尚未完全合并到 b 分支"
fi
```
